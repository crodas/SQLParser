...
join_type(A) ::= join_prefix(B) T_RIGHT join_postfix(C).      { A = new Stmt\Join('RIGHT', B, C); }
...
cast(A) ::= T_CAST T_PAR_OPEN expr(B) T_AS type(X) T_PAR_CLOSE. { A = new Stmt\Expr('cast', B, X); }
type(A) ::= T_INTEGER(X) . { A = X; }
...
term(A) ::= T_INTERVAL expr(C) T_ALPHA(X).  { A = new Stmt\Expr('timeinterval', C, X); }
...
function_call(A) ::= T_ALPHA(C) expr_list_par_or_null(D) . {
...
alpha(A) ::= T_INTERVAL(X) .      { A = X; }
alpha(A) ::= T_AUTO_INCREMENT(X). { A = X; }
alpha(A) ::= T_ALPHA(B).          { A = B; }
...
variable_name(A) ::= T_ALPHA(X) . { A = X; }
variable_name(A) ::= T_LIMIT|T_INSERT|T_UPDATE|T_FROM|T_SELECT|T_COLLATE|T_AUTO_INCREMENT|T_DEFAULT|T_PRIMARY|T_OFFSET|T_KEY(X) . { A = X; }
